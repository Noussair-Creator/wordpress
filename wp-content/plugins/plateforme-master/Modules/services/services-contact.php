...
if (!function_exists('svc_contacts_table')) {
  function svc_contacts_table()
  {
    global $wpdb;
    return $wpdb->prefix . 'contacts';
  }
}
if (!function_exists('svc_contact_membre_table')) {
  function svc_contact_membre_table()
  {
    global $wpdb;
...
if (!function_exists('svc_contact_laboratoire_table')) {
  function svc_contact_laboratoire_table()
  {
    global $wpdb;
...
if (!function_exists('svc_contact_list')) {
  ...
  {
    global $wpdb;
    $t = svc_contacts_table();
    $lab_ids = svc_contact_current_user_lab_ids();
    if (empty($lab_ids))
      return array(); // aucun labo => aucune donnée
...
if (!function_exists('svc_contact_get')) {
  function svc_contact_get(WP_REST_Request $req)
  {
    ...
    if (!$row)
      return new WP_Error('not_found', 'Introuvable', array('status' => 404));
...
if (!function_exists('svc_contact_create')) {
  function svc_contact_create(WP_REST_Request $req)
  {
    ...
    $uid = get_current_user_id();
    if (svc_contact_col_exists($t, 'created_by')) {
      $ins['created_by'] = $uid;
      $fmts[] = '%d';
    }
    if (svc_contact_col_exists($t, 'updated_by')) {
      $ins['updated_by'] = $uid;
      $fmts[] = '%d';
    }
    if (svc_contact_col_exists($t, 'created_at')) {
      $ins['created_at'] = $now;
      $fmts[] = '%s';
    }
    if (svc_contact_col_exists($t, 'updated_at')) {
      $ins['updated_at'] = $now;
      $fmts[] = '%s';
    }
...
if (!function_exists('svc_contact_update')) {
  function svc_contact_update(WP_REST_Request $req)
  {
    ...
    if (empty($upd))
      return new WP_Error('bad_request', 'Aucun champ valide', array('status' => 400));
...
{
  if (!is_string($dataurl) || stripos($dataurl, 'data:image') !== 0)
    return $dataurl;
