...
<body>
    ...
    <div class="content-wrapper">
        ...
        <div class="card full-width">
            <h3>Informations générales
                ...
                <a href="#" id="modifyInfoBtn"><i class="fa-regular fa-pen-to-square" style="color: #b60303;"></i></a>
    ...
    /* 2) Endpoints sûrs */
    const API_LABO = restEndpoint(
        'plateforme-recherche/v1/laboratoire'); // ex: https://.../wp-json/plateforme-recherche/v1/laboratoire
    /*
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        try {
            ...
            if (etabRow) etabRow.innerHTML =
                `<strong>Établissement :</strong> ${labo.etablissement_nom || ''}`;

            // --- Date création ---
            const dateRow = document.querySelector('.styled-list li:nth-child(4)');
            if (dateRow) dateRow.innerHTML =
                `<strong>Date de création :</strong> ${labo.date_creation || ''}`;

            // --- Directeur (nom complet depuis usermeta) ---
            const dirRow = document.querySelector('.styled-list li:nth-child(5)');
            if (dirRow) dirRow.innerHTML =
                `<strong>Directeur du laboratoire :</strong> ${labo.directeur_nom_complet || labo.directeur_nom || ''}`;
            ...
            if (statutRow) {
                ...
                statutRow.innerHTML =
                    `<strong>Statut du financement :</strong>
        <span><i class="fas fa-circle status-active-icon" style="color:${actif ? '#28a745' : '#dc3545'}"></i>${labo.statut}</span>`;
            ...
            if (objRow) objRow.innerHTML =
                `<strong>Objectif général :</strong> ${labo.objectif_general || ''}`;
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        (async function preload() {
            try {
                const url = new URL('plateforme-recherche/v1/laboratoire/mine', PMSettings.restUrl)
                    .href;
