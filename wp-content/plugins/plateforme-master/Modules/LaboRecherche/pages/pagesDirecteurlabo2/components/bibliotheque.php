...
<div class="content-block">
    ...
    <table id="candidaturesTable" class="styled-table display">
        ...
        <tbody>
            <tr>
                ...
                <td class="text-center">
                    <a href="#"><img width="20px"
                            ...
                            alt="Groupe 152.png"></a>
                </td>
            </tr>
            <tr>
                <td><input type="checkbox"></td>
                <td>Modèles-de-rapport.pdf</td>
                <td>Modèles De Rapport</td>
                <td>2023-2024</td>
                <td>Dr. Trabelsi</td>
                <td class="text-center">
                    <a href="#"><img width="20px"
            ...
            <tr>
                <td><input type="checkbox"></td>
                <td>Convention-du-stage.pdf</td>
                <td>Supports Administratifs</td>
                <td>2023-2024</td>
                <td>Directeur UTM</td>
                <td class="text-center">
                    <a href="#"><img width="20px"
            ...
            <tr>
                <td><input type="checkbox"></td>
                <td>Guide-des-soutenances.pdf</td>
                <td>Guides Étudiants</td>
                <td>2023-2024</td>
                <td>Mm. Soufia Ben Nejma</td>
                <td class="text-center">
                    <a href="#"><img width="20px"
                            src="/wp-content/plugins/plateforme-master/images/icons/Groupe 152.png"
                            alt="Groupe 152.png"></a>
...
<div class="modal-overlay" id="addMeetingModal">
    <div class="popup-container">
        ...
        <form class="popup-form">
            ...
            <div class="form-group">
                <label for="addedBy">Ajouter Par</label>
                <input type="text" id="addedBy" placeholder="Ajouter Par...">
            </div>
            <div class="form-group">
                <label for="addDate">Date D'ajout</label>
                <div class="input-with-icon">
                    <input type="text" id="addDate" placeholder="Date D'ajout" onfocus="(this.type='date')">
...
<div class="modal-overlay" id="editMeetingModal">
    <div class="popup-container">
        <div class="popup-header">
            ...
            <div class="header-actions">
                <button class="btn-enregistrer" id="updateMeetingBtn">Mettre à jour</button>
                <button class="btn-close-x" id="closeEditModalBtn">×</button>
            </div>
        </div>
        <form class="popup-form">
            <input type="hidden" id="editMeetingId">
            <div class="form-group">
                <label for="editMeetingSubject">Sujet</label>
                <input type="text" id="editMeetingSubject" placeholder="Sujet">
            </div>
            <div class="form-group">
                <label for="editMeetingDate">Date</label>
                <div class="input-with-icon">
                    <input type="text" id="editMeetingDate" placeholder="Date" onfocus="(this.type='date')">
                    <i class="fas fa-calendar-alt icon right-icon"></i>
                </div>
            </div>
            <div class="form-group">
                <label for="editParticipantEmail">Ajouter un participant</label>
                <div class="input-with-icon">
                    <input type="email" id="editParticipantEmail" placeholder="Email Participant">
                    <i class="fas fa-level-down-alt icon right-icon"
                        style="transform: translateY(-50%) rotate(90deg); cursor: pointer;"
                        id="addEditParticipantBtn"></i>
                </div>
            </div>
...
<div class="modal-overlay" id="detailsMeetingModal">
    <div class="popup-container">
        <div class="popup-header">
            ...
            <div class="header-actions">
                <button class="btn-close-x" id="closeDetailsModalBtn">×</button>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    saveBtn.addEventListener('click', () => {
        // You can add logic here to get form values and add a new row to the table
        closeAddModal();
    });
    ...
    $('#candidaturesTable').on('click', '.edit-btn', function(e) {
        ...
        // Updated selectors to match new table structure
        const fileName = row.find('td:eq(1)').text();
        const year = row.find('td:eq(3)').text();

        // Map file data to meeting modal (as an example)
        $('#editMeetingSubject').val(fileName);
        $('#editMeetingDate').val(''); // Year doesn't directly map to a date
    ...
    updateBtn.addEventListener('click', () => {
        // This logic is now less relevant to the document table, but kept for functionality
        const subject = $('#editMeetingSubject').val();
        const date = $('#editMeetingDate').val();
        const rowIndex = $('#editMeetingId').val();
        table.cell(rowIndex, 1).data(subject).draw(); // Update Nom du fichier
        // table.cell(rowIndex, 3).data(date).draw(); // No direct mapping for date
        closeEditModal();
    });
    ...
    const detailsPopupContainer = detailsModal.querySelector('.popup-container');
    ...
    $('#candidaturesTable').on('click', '.details-btn', function(e) {
        ...
        const addedBy = row.find('td:eq(4)').text();
        ...
        // Map file data to details modal
        $('#detailsSubject').text(fileName);
        $('#detailsDate').text(type);
        $('#detailsDuration').text(`Ajouté par: ${addedBy}`);

        const participantList = $('#detailsParticipantList');
        participantList.empty(); // Clear previous list

        mockParticipants.forEach(p => {
            const listItem =
                `<li><img src="${p.avatar}" alt="Avatar"><span>${p.name}</span></li>`;
    ...
    detailsModal.addEventListener('click', (e) => {
        if (!detailsPopupContainer.contains(e.target)) {
            closeDetailsModal();
        }
    });
});
