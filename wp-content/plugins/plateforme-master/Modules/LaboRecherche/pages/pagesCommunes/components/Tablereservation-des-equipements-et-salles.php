...
<body>
          ...
          <table class="styled-table" id="reservationsTable">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>01/05/2025</td>
                <td>13:30 - 14:00</td>
                <td><span class="badge badge-warning"><i class="fa-regular fa-clock" style="color: #d89e00;"></i>En
                    attente</span></td>
                <td>
                  <div class="actions">
                    <button class="action-btn">...</button>
        ...
        <div class="date-time-group">
          <div class="form-group">
            ...
            <div class="input-with-icon">
              ...
              <img width="20" src="/wp-content/plugins/plateforme-master/images/icons/27) Icon-calendar.png"
                alt="Icon-calendar">
            </div>
          </div>
          <div class="form-group">
      ...
      <form class="popup-form">
        ...
        <div class="form-group">
          ...
          <div class="input-file-wrapper">
            ...
            <label class="btn-importer"><i class="fa fa-upload"></i> Importer <input type="file" multiple
    ...
    <div class="popup-container" id="popupContainerModifierEquipement">
      ...
      <form class="popup-form">
        ...
        <div class="form-group">
          ...
          <div class="existing-images" style="margin-top: 10px; display: flex; flex-wrap: wrap; gap: 10px;">
          </div>
        </div>
        <div class="form-group">
        ...
        <div class="detail-item">
          ...
          <div class="file-download-link">
            ...
            <img width="20px" class="download-icon"
              src="/wp-content/plugins/plateforme-master/images/icons/Groupe 152.png" alt="upload-red.png">
          </div>
        </div>
        <div class="detail-item">
          <label>Contrat :</label>
  ...
  <script>
    $(document).ready(function () {
      ...
      function _inferTypeFromLabel(label = '') {
        ...
        return (s.startsWith('salle') || s.includes('amphi') || s.includes('auditorium')) ? 'Salle' :
          'Équipement';
      }
      ...
      function _statusBadge(statut = '') {
        ...
        if (s === 'validee')
          return `<span class="badge badge-success"><i class="fa-regular fa-circle-check"></i> Validée</span>`;
        if (s === 'refusee')
          return `<span class="badge badge-danger"><i class="fa-regular fa-circle-stop"></i> Refusée</span>`;
        if (s === 'annulee')
          return `<span class="badge badge-danger"><i class="fa-regular fa-circle-stop"></i> Annulée</span>`;
      ...
      function statusBadgeFR(s) {
        ...
        const v = String(s || '').toLowerCase();
        if (v === 'validee')
          return `<span class="badge badge-success"><i class="fa-regular fa-circle-check"></i> ${lbl}</span>`;
        if (v === 'refusee')
          return `<span class="badge badge-danger"><i class="fa-regular fa-circle-stop"></i> ${lbl}</span>`;
        if (v === 'annulee')
          return `<span class="badge badge-danger"><i class="fa-regular fa-circle-stop"></i> ${lbl}</span>`;
  ...
  `;
      ...
      async function reloadReservations() {
        try {
          ...
          (rows || []).forEach(r => {
            ...
            const nomTxt =
              (r.equip_nom ?
                ...
                (r.resource_label && r.resource_label !== '0' ? r.resource_label :
                  `#${r.resource_id}`));
          ...
          window.toast?.('Erreur chargement des réservations', true) || alert(
      ...
      $(document)
        ...
        .on('click.openEditEquip', '.openModifierEquipementModal', async function (e) {
          ...
          if (!id) {
            ...
            window.toast?.("ID d'équipement introuvable", true) || alert(
              "ID d'équipement introuvable");
            return;
          }
          ...
          try {
            ...
            console.error(err);
            window.toast?.('Impossible de charger les données', true) || alert(
      ...
      async function openModifierEquipementModal(id) {
        ...
        const [protoUrl, entretien] = await Promise.all([
          ...
          fetchEntretienByEquip(id).catch(() =>
            null) // { periodicite, consignes, fichier_contrat }
        ]);
        ...
        */
        ...
        $m.find('.input-file-wrapper input[type="file"]').off('change.mirror').on('change.mirror',
          function () {
  ...
  <script>
    ...
    async function loadEquipementLookups() {
      try {
        ...
        fillSelectWithPreserve(document.getElementById('modifier-categorie-equipement'), cats,
          'Catégorie Des Equipements', true);
        fillSelectWithPreserve(document.getElementById('modifier-disponibilite-appareil'), dispos,
          'Disponibilité', true);
    ...
    function $modalEdit() {
      return $('#modalModifierEquipement');
    }

    function $wrapEdit() {
      return $('#modalModifierEquipement .popup-form');
    }
    ...
    async function postConditionsEntretien(equipementId, {
      periodicite,
      consignes,
      fichier_contrat
    }) {
    ...
    function resetEquipementForm() {
      ...
      form.querySelectorAll('.input-file-text').forEach(el => {
        el.value = '';
      });
    ...
    async function createEquipement() {
      ...
      const loaderOff = () => {
        if (btn) {
          btn.disabled = false;
          btn.textContent = btn.dataset._txt || 'Enregistrer';
        }
      };
    ...
    function statutIconHTML(statut) {
      switch (String(statut || '').toLowerCase()) {
        case 'fonctionnel':
          return '<i class="fa fa-check-circle" style="color:#A6A485;"></i>';
        case 'en_panne':
          return '<i class="fa-solid fa-triangle-exclamation" style="color:#BF0404;"></i>';
        case 'en_maintenance':
          return '<i class="fa-solid fa-screwdriver-wrench" style="color:#DDACA7;"></i>';
        case 'hors_service':
          return '<i class="fa fa-times-circle" style="color:#888;"></i>';
        default:
          return '<i class="fa fa-minus-circle" style="color:#ccc;"></i>';
      }
    }
  ...
  `;
    ...
    async function fetchLastMaintenance(equipementId) {
      ...
      const url =
        `${REST_BASE}/plateforme-directeurderecherche/v1/maintenance?equipement_id=${encodeURIComponent(equipementId)}&per_page=1`;
    ...
    async function getProtocoleURL(equipementId) {
      const url =
        `${REST_BASE}/plateforme-directeurderecherche/v1/equipement_protocole?equipement_id=${encodeURIComponent(equipementId)}&per_page=1`;
      const rows = await getJSON(url);
  ...
  `;
    ...
    async function reloadEquipements() {
      ...
      if (!$tbl.length) {
        console.warn('[reloadEquipements] Table absente');
        return;
      }
          ...
          if (!dtEventsBound) {
            ...
            $('#equipementsTable tbody')
              ...
              .on('click', '.openModifierEquipementModal', function (e) {
                e.preventDefault();
                const id = $(this).data('id');
                if (typeof window.openModifierEquipementModal === 'function') window
                  .openModifierEquipementModal(id);
              })
              ...
              .on('click', '.action-supprimer', async function (e) {
                ...
                try {
                  ...
                  window.toast?.('Équipement supprimé') || alert('Supprimé');
                  reloadEquipements();
                } catch (err) {
                  console.error(err);
                  window.toast?.('Erreur suppression', true) || alert('Erreur suppression');
                }
              });
    ...
    async function fetchEntretienByEquip(id) {
      ...
      try {
        const rows = await getJSON(
          `${REST_BASE}/plateforme-directeurderecherche/v1/conditions_entretien?equipement_id=${encodeURIComponent(id)}&per_page=1`
        );
  ...
  `;
    ...
    window.openEquipementDetailsModal = async function (id) {
      try {
        ...
        window.toast?.('Impossible de charger les détails', true) || alert(
    ...
    window.openModifierEquipementModal = async function (id) {
      try {
        ...
        window.toast?.('Impossible de charger l’équipement', true) || alert(
    ...
    async function updateEquipementFromEditModal() {
      ...
      // fichiers choisis
      const protoFile = $m.find('.input-file-text[placeholder="Protocole d\'utilisation"]').closest(
        '.input-file-wrapper').find('input[type="file"]')[0]?.files?.[0] || null;
      const contrFile = $m.find('.input-file-text[placeholder="Contrat"]').closest('.input-file-wrapper').find(
      ...
      const $imagesWrap = $(
        '#modalModifierEquipement .input-file-wrapper:has(input[placeholder="Importer des images..."])');
      ...
      const $btn = $m.find('.btn-enregistrer');
      const txt = $btn.text();
      ...
      try {
        ...
        if (protoFile) {
          const protoUrl = await uploadMedia(protoFile,
            `Protocole - ${payload.nom_appareil || 'Equipement'}`);
          if (protoUrl) await postEquipementProtocole(id, protoUrl);
        }
        let contrUrl = '';
        if (contrFile) contrUrl = await uploadMedia(contrFile,
          `Contrat - ${payload.nom_appareil || 'Equipement'}`);
    ...
    function $formMaint() {
      return $('#modalMaintenance .popup-form');
    }
    ...
    window.openMaintenanceModal = async function (equipementId) {
      try {
        ...
        window.toast?.('Impossible d’ouvrir la maintenance', true) || alert(
          'Impossible d’ouvrir la maintenance');
      }
    };
    ...
    async function uploadIfAny($wrapper) {
      ...
      return uploadMedia(file, title).catch(e => {
    ...
    async function saveMaintenanceFromModal() {
      ...
      if (!equipId) {
        window.toast?.('Équipement manquant', true) || alert('Équipement manquant');
        return;
      }
      ...
      try {
        ...
        window.toast?.('Demande de maintenance enregistrée', false) || alert(
          'Demande de maintenance enregistrée');
    ...
    $('#popupContainerMaintenance .btn-enregistrer')
      .off('click.saveMaint')
      .on('click.saveMaint', function (e) {
        e.preventDefault();
        saveMaintenanceFromModal();
      });
    ...
    async function fetchMaintenancesByEquip(equipementId, per = 200) {
      const url =
        `${REST_BASE}/plateforme-directeurderecherche/v1/maintenance?equipement_id=${encodeURIComponent(equipementId)}&per_page=${per}`;
      return getJSON(url);
    }
    ...
    async function openMaintenanceHistoryModal(equipementId) {
      try {
        ...
        window.toast?.('Impossible de charger l’historique', true) || alert(
    ...
    $(document)
      ...
      .on('click.actionSupprimerEquip', '.action-supprimer', async function (e) {
        ...
        // Confirmation
        if (!confirm(
          'Supprimer cet équipement et toutes ses correspondances (protocoles, entretiens, maintenances) ?'
        )) {
          return;
        }
    ...
    */
    ...
    async function saveReservationFromModal() {
      ...
      const date = ($('#date-reservation').val() || '').split('/').reverse().join(
        '-'); // 11/01/2025 -> 2025-01-11
  ...
  `;
    ...
    window.reloadReservations = async function () {
      try {
        // deux vues utiles :
        // 1) Mes réservations
        const mine = await fetchReservations({
          mine: 1,
          per_page: 200
        });
        // 2) Réservations sur MES équipements
        const owner = await fetchReservations({
          owner: 1,
          per_page: 200
        });
    ...
    $(document)
      ...
      .on('click.res.appr', 'a.res-approve', async function (e) {
        e.preventDefault();
        const id = $(this).data('id');
        try {
      ...
      .on('click.res.rej', 'a.res-reject', async function (e) {
        e.preventDefault();
        const id = $(this).data('id');
        try {
      ...
      .on('click.res.can', 'a.res-cancel', async function (e) {
        e.preventDefault();
        const id = $(this).data('id');
        if (!confirm('Annuler cette réservation ?')) return;
        try {
    ...
    $(document).off('click.res.closemenus').on('click.res.closemenus', function () {
      $('.dropdown-menu').hide();
    });
    ...
    async function openReservationEditModal(id) {
      try {
        ...
        $('#modifier-heure-reservation').val(
          `${(r.heure_debut || '').slice(0, 5)} - ${(r.heure_fin || '').slice(0, 5)}`);
        ...
        window.toast?.('Impossible de charger la réservation', true) || alert(
    ...
    async function updateReservationFromEditModal() {
      ...
      try {
        const res = await fetch(
          `${REST_BASE}/plateforme-directeurderecherche/v1/reservation/${encodeURIComponent(id)}`, {
          method: 'PATCH',
          headers: HEADERS_JSON,
          credentials: 'include',
          body: JSON.stringify({
        ...
        if (String(e.message).includes('CONFLICT')) {
          window.toast?.('Créneau non disponible pour cet équipement', true) || alert(
  ...
  <script>
    ...
    async function apiFetchTopRessources(yearLabel, limit = 8) {
      const year = encodeURIComponent(parseYearRangeLabel(yearLabel));
      return getJSON(
        `${REST_BASE}/plateforme-directeurderecherche/v1/top-ressources?year=${year}&limit=${limit}`);
    }
    ...
    function renderBarChart(rows) {
      ...
      const colors = values.map((_, i) => ['#A6A485', '#DDACA7', '#FFD54F', '#A6C7FF', '#BF0404', '#808066',
        '#b1342f', '#dabebe'
      ][i % 8]);
