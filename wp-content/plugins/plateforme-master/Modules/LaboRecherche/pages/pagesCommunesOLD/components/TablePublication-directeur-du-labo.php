            ...
            <div class="table-controls">
                <div class="filter-group">
                    ...
                    <div class="seg" role="tablist" aria-label="Filtre portée">
                        ...
                        <button type="button" class="seg-btn" data-scope="shared" aria-selected="false">Articles
                            Partagés</button>
                    </div>
                </div>
...
(function($) {
    /* ====== Config REST ====== */
    const REST_ROOT = (window.pmsettings && pmsettings.rest_root) || (window.wpApiSettings && wpApiSettings.root) ||
        '/wp-json/';
    ...
    async function loadMesPublications() {
        ...
        `;
            ...
            if (canModerate) {
                if (!editAlreadyAdded) {
                    actionsHtml +=
                        `<a href="${editHrefMine}"><i class="fa-regular fa-pen-to-square"></i>Modifier</a>`;
                    editAlreadyAdded = true;
                }
                actionsHtml += `
        <a href="#" class="js-validate" data-id="${esc(p.id)}"><i class="fa-regular fa-circle-check"></i>Valider</a>
    ...
    $(document).on('click', '.js-publish', async function(e) {
        ...
        try {
            ...
            console.error(err);
            alert("Publication impossible (erreur serveur).");
        }
    });
    ...
    $(document).on('click', '.js-del-pub', async function(e) {
        ...
        try {
            ...
            if (!res.ok) throw new Error('HTTP ' + res.status);
            if (dtMes) dtMes.row($tr).remove().draw(false);
            else $tr.remove();
        } catch (err) {
            console.error(err);
    ...
    function broadcastStatus(id, statut) {
        ...
        try {
            localStorage.setItem('pub_status_event', JSON.stringify({
                ...payload.payload,
                ts: Date.now()
            }));
        } catch (_) {}
