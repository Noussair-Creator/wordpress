...
window.PMOPagination = {
    ...
    init: function(dataTable) {
        ...
        // Configure DataTable to hide default pagination
        if (dataTable) {
            dataTable.settings()[0].oInit.dom = '<"top">rt<"clear">';
            dataTable.settings()[0].oInit.drawCallback = function() {
                PMOPagination.update();
            };
            dataTable.settings()[0].oInit.initComplete = function() {
                PMOPagination.update();
            };
        }
    ...
    bindEvents: function() {
        ...
        if (firstBtn) {
            firstBtn.onclick = function(e) {
                e.preventDefault();
                if (PMOPagination.currentDataTable && !firstBtn.disabled) {
                    PMOPagination.currentDataTable.page('first').draw('page');
                }
            };
        }
        ...
        if (prevBtn) {
            prevBtn.onclick = function(e) {
                e.preventDefault();
                if (PMOPagination.currentDataTable && !prevBtn.disabled) {
                    PMOPagination.currentDataTable.page('previous').draw('page');
                }
            };
        }
        ...
        if (nextBtn) {
            nextBtn.onclick = function(e) {
                e.preventDefault();
                if (PMOPagination.currentDataTable && !nextBtn.disabled) {
                    PMOPagination.currentDataTable.page('next').draw('page');
                }
            };
        }
        ...
        if (lastBtn) {
            lastBtn.onclick = function(e) {
                e.preventDefault();
                if (PMOPagination.currentDataTable && !lastBtn.disabled) {
                    PMOPagination.currentDataTable.page('last').draw('page');
                }
            };
        }
    },
    ...
    update: function() {
        if (!this.currentDataTable) return;

        try {
            const info = this.currentDataTable.page.info();
            const firstBtn = document.getElementById('firstPageBtn');
            const prevBtn = document.getElementById('prevPageBtn');
            const nextBtn = document.getElementById('nextPageBtn');
            const lastBtn = document.getElementById('lastPageBtn');
