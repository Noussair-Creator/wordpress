...
?>
  ...
  <style>
    ...
    #suivi-rec col.c1{width:4%}
    #suivi-rec col.c2{width:16%}
    #suivi-rec col.c3{width:20%}
    #suivi-rec col.c4{width:30%}
    #suivi-rec col.c5{width:10%}
    #suivi-rec col.c6{width:12%}
    #suivi-rec col.c7{width:8%}

    .head-box{ background:#ECEBE3; border:1px solid #A6A4853D; border-radius:8px; overflow:hidden; }
    #suivi-rec .sr-table-head thead th{ background:transparent; padding:10px 10px; text-align:left; font:700 15px/20px Roboto; color:var(--ink);}
    ...
    .body-box{ border:2px solid var(--line); border-radius:8px; background:#fff; margin-top:10px; }
    #suivi-rec .sr-table-body tbody td{ padding:10px 10px; vertical-align:middle; background:#fff; border-top:1px solid var(--edge); }
    ...
    #suivi-rec .sr-dots{ width:28px;height:28px;border-radius:6px;background:transparent;border:1px solid transparent;cursor:pointer; font-size:22px; line-height:26px; }
    #suivi-rec .sr-dots:hover{ background:#f7f6f1; border-color:#e8e3cf; }

    #suivi-rec .sr-menu{ position:absolute; top:30px; right:0; background:#fff; border:1px solid #E6E4D8; border-radius:10px; box-shadow:0 8px 24px rgba(0,0,0,.12); display:none; min-width:180px; padding:6px; z-index:10; }
    #suivi-rec .sr-menu a{ display:flex; align-items:center; gap:10px; padding:8px 10px; border-radius:8px; text-decoration:none; color:#2A2916; font:500 13px/1.2 Roboto; }
    #suivi-rec .sr-menu a:hover{ background:#F7F6F2; }
    #suivi-rec .sr-menu .ico{ width:18px; height:18px; display:inline-block; flex:0 0 18px; background-size:18px 18px; background-repeat:no-repeat; background-position:center; }
    #suivi-rec .ico-doc{  background-image:url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" stroke="%232A2916" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v10.5A1.5 1.5 0 0 0 5.5 16H14a2 2 0 0 0 2-2V6z"/><path d="M14 2v4h4"/></svg>'); }
    #suivi-rec .ico-mail{ background-image:url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" stroke="%232A2916" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M4 4h12a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2z"/><path d="m22 6-10 7L2 6"/></svg>'); }

    /* Pagination */
    #suivi-rec .sr-pager{ position:absolute; right:16px; bottom:12px; width:auto; height:27px; display:flex; align-items:center; gap:12px; }
    #suivi-rec .sr-btn{ width:27px; height:27px; border:2px solid var(--danger); border-radius:3px; background:#fff; display:inline-flex; align-items:center; justify-content:center; cursor:pointer; box-sizing:border-box; font-weight:700; color:#BF0404; line-height:1; font-size:22px; padding:0; }
    ...
    #suivi-rec .sr-sidebar{ position:fixed; top:0; right:0; width:450px; height:100vh; background:#FFFFFF; box-shadow:-7px 0px 36px #00000029; transform:translateX(110%); transition:.25s; z-index:9999; display:flex; flex-direction:column; border-radius:0; }
    #suivi-rec .sr-sidebar.open{ transform:translateX(0) }
    #suivi-rec .sr-sb-head{ height:60px; background:#FFFFFF; box-shadow:0px 5px 16px #00000029; display:flex; align-items:center; justify-content:space-between; padding:10px 12px; }
    #suivi-rec .sr-sb-title{ font:700 18px/24px Roboto; color:#2A2916; }
    #suivi-rec .sr-sb-close{ width:40px; height:40px; border:none; border-radius:5px; cursor:pointer; background:#BF0404 url('/wp-content/plugins/plateforme-master/images/icon%20etudiant/27)%20Icon-close.png') center/14px 14px no-repeat; }
    ...
    #suivi-rec .sr-form .sr-field{margin-bottom:12px}
    #suivi-rec .sr-form label{display:block;font:700 13px/1.2 Roboto;color:#2A2916;margin-bottom:6px}
    #suivi-rec .sr-form select,#suivi-rec .sr-form textarea{width:100%;box-sizing:border-box;border:1px solid #DBD9C3;border-radius:6px;padding:10px;background:#fff;font:400 14px/1.2 Roboto}
    #suivi-rec .sr-form textarea{min-height:120px;resize:vertical}
    #suivi-rec .sr-form .sr-actions{display:flex;gap:10px;justify-content:flex-end;margin-top:10px}
    #suivi-rec .sr-btn-red{background:#BF0404;color:#fff;border:none;border-radius:6px;height:38px;padding:0 14px;font:700 13px/1 Roboto;cursor:pointer}
    #suivi-rec .sr-btn-ghost{background:#fff;color:#BF0404;border:2px solid #BF0404;border-radius:6px;height:38px;padding:0 14px;font:700 13px/1 Roboto;cursor:pointer}
  ...
  <div class="sr-card">
    ...
    <div class="sr-toolbar">
      ...
      <div style="margin-left:auto;display:flex;gap:10px">
        <a class="sr-btn-primary" id="sr-add-btn" href="<?php echo esc_url($reclamations_url); ?>" target="_self">
    ...
    <div class="sr-pager">
      ...
      <span class="sr-num" id="sr-page">1</span>
...
<?php endif; ?>
  ...
  (function(){
    ...
    const pageNum  = root.querySelector('#sr-page');
    const addBtn   = root.querySelector('#sr-add-btn');

    const LIST_API = "<?php echo esc_url( rest_url('plateforme/v1/reclamations') ); ?>";
    const PUT_REP  = (id)=> "<?php echo esc_url( rest_url('plateforme/v1/reclamations') ); ?>"+"/"+id+"/reponse";
    ...
    function row(d, i){
      const isOwner = (d.owner_user_id === CURRENT_USER);
      const menuOwner = `
        <a href="#" data-act="view" data-i="${i}"><span class="ico ico-doc"></span> Voir</a>
        <a href="#" data-act="email" data-i="${i}"><span class="ico ico-mail"></span> E-mail</a>
      `;
      const menuOther = `
        <a href="#" data-act="view" data-i="${i}"><span class="ico ico-doc"></span> Voir</a>
        <a href="#" data-act="reply" data-i="${i}"><span class="ico ico-mail"></span> Ajouter une réponse</a>
      `;
        ...
        <tr data-i="${i}">
          ...
          <td class="sr-actions">
            ...
            <div class="sr-menu" role="menu">
              ${isOwner ? menuOwner : menuOther}
            </div>
          </td>
        </tr>`;
    }

    function draw(list){ tbody.innerHTML = list.map(row).join(''); bindRowEvents(); }

    let DATA = []; let PAGE = 1; const PER = 10; let CURRENT_INDEX = -1;

    async function load({search='', page=1} = {}){
      PAGE = page; pageNum.textContent = page;
      const url = new URL(LIST_API);
      url.searchParams.set('page', page);
      url.searchParams.set('per_page', PER);
      if (search) url.searchParams.set('search', search);

      const res = await fetch(url.toString(), { headers: { 'X-WP-Nonce': NONCE }, credentials: 'same-origin' });
    ...
    function openOverlay(){ ov.classList.add('open'); }
    function closeOverlay(){ ov.classList.remove('open'); closeSidebars(); }
    function closeSidebars(){ sbV.classList.remove('open'); sbR.classList.remove('open'); }
    ...
    function openDetail(i){
      ...
      `;
      openOverlay(); sbV.classList.add('open');
    }
    ...
    function openReply(i){
      ...
      openOverlay(); sbR.classList.add('open');
    }

    // Envoi réponse
    repForm.addEventListener('submit', async (e)=>{
      e.preventDefault();
      const i = CURRENT_INDEX; const d = DATA[i]; if(!d) return;
      ...
      if (!res.ok) { alert(out && out.message ? out.message : 'Erreur lors de l’enregistrement.'); return; }
