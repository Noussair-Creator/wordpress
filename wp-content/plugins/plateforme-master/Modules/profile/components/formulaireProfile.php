        ...
        <?php else: ?>
          ...
          <div class="address-block" id="acadBlock">
            ...
            <div class="address-row" style="margin-top:12px">
              ...
              <div class="phone">
                <div class="field" id="proCountryBox" style="position:relative;">
                  <img class="flag" id="proFlag" src="https://flagcdn.com/w20/tn.png" alt="TN">
                  <i class="fa-solid fa-chevron-down chev"></i>
                  <div class="country-menu" id="proCountryMenu">
                    <button type="button" data-iso="tn"><img class="flag" src="https://flagcdn.com/w20/tn.png" alt=""> Tunisie</button>
                    <button type="button" data-iso="fr"><img class="flag" src="https://flagcdn.com/w20/fr.png" alt=""> France</button>
                    <button type="button" data-iso="ma"><img class="flag" src="https://flagcdn.com/w20/ma.png" alt=""> Maroc</button>
                    <button type="button" data-iso="dz"><img class="flag" src="https://flagcdn.com/w20/dz.png" alt=""> Algérie</button>
                ...
                <div class="field">
                  <input type="text" name="pro_tel" id="pro_tel" placeholder="Téléphone Professionnel">
                </div>
              </div>
            </div>
...
form.addEventListener('submit', async (e) => {
  ...
  const payload = {
    ...
    nationalite: (natHidden?.value || natSelect?.value || '').trim(),
    cin: cinInput.value.trim(),

    // (optionnel) si tu veux que le service mette à jour prénom/nom
  ...
  if (IS_STUDENT) {
    Object.assign(payload, {
      adr_etud: adrEtudInput?.value?.trim() || '',
      gov_etud: govEtudSelect?.value || '',
      cp_etud:  cpEtudInput?.value?.trim() || '',
      adr_parents: adrParInput?.value?.trim() || '',
      gov_parents: govParSelect?.value || '',
      cp_parents:  cpParInput?.value?.trim() || '',
      tel_parents: telParInput?.value?.trim() || ''
    });
  } else {
    const gradeId = gradeSel?.value || '';
    const specId  = specSel?.value || '';
    payload.grade_id = gradeId ? parseInt(gradeId, 10) : 0;
    payload.specialite_id = specId ? parseInt(specId, 10) : 0;

    payload.academic_info = {
      email_acad: document.getElementById('acad_email')?.value?.trim() || '',
      tel_pro_country: (document.getElementById('proFlag')?.alt || 'TN').toLowerCase(),
...
// Pro phone
const proCountryBox = document.getElementById('proCountryBox');
const proCountryMenu = document.getElementById('proCountryMenu');
const proFlag = document.getElementById('proFlag');
proCountryBox?.addEventListener('click', () => {
  if (!proCountryMenu) return;
  proCountryMenu.style.display = (proCountryMenu.style.display === 'block' ? 'none' : 'block');
});
proCountryMenu?.querySelectorAll('button').forEach(btn => {
  btn.addEventListener('click', () => {
    proFlag.src = `https://flagcdn.com/w20/${btn.dataset.iso}.png`;
    proFlag.alt = btn.dataset.iso.toUpperCase();
    proCountryMenu.style.display = 'none';
  });
});
document.addEventListener('click', e => {
  if (proCountryBox && proCountryMenu && !proCountryBox.contains(e.target)) proCountryMenu.style.display = 'none';
});
