...
document.addEventListener('click', (e) => {
    if (!msgBtn.contains(e.target)) toggleMsg(false);
});
document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') toggleMsg(false);
});
...
(function() {
    ...
    async function api(path, {
        ...
        if (!r.ok) throw Object.assign(new Error(j?.message || ('HTTP ' + r.status)), {
            status: r.status,
            detail: j
        });
        return j;
    ...
    async function updateNavbarMessages() {
        try {
            ...
            const latest = await api('/messages/threads', {
                method: 'GET',
                query: {
                    limit: 5,
                    offset: 0
            ...
            const unreadThreads = await api('/messages/threads', {
                method: 'GET',
                query: {
                    only_unread: 1,
                    limit: 200
            ...
            const totalUnread = (unreadThreads || []).reduce((acc, t) => acc + (parseInt(t.unread_count || 0,
                10) || 0), 0);
            ...
            if (listEl.innerHTML.trim() === '') {
                listEl.innerHTML =
                    `<div class="msg-item"><div class="msg-snippet">Erreur de chargement</div></div>`;
            }
        }
    }
