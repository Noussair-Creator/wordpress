...
function svc_projet_create(WP_REST_Request $req) {
  ...
  function svc_reseaux_update(WP_REST_Request $req)
  {
    ...
    // Handle image URLs
    if (isset($req['logo_url']))    $data['logo_url']    = esc_url_raw($req['logo_url']);
    if (isset($req['avatar_url']))  $data['avatar_url']  = esc_url_raw($req['avatar_url']);

    // Handle additional contact fields
    if (isset($req['contact_tel']))     $data['contact_tel']     = sanitize_text_field($req['contact_tel']);
    if (isset($req['contact_fonction'])) $data['contact_fonction'] = sanitize_text_field($req['contact_fonction']);
