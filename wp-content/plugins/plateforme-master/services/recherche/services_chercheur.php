...
{
  global $wpdb;
  return $wpdb->prefix . 'recherche_laboratoire';
}
...
function svc_labo_sanitize($key, $val, $def)
{
  ...
  switch ($type) {
    ...
    case 'enum':
      return in_array($val, $def[1] ?? array(), true) ? $val : null;
    case 'json':
      return is_string($val) ? $val : wp_json_encode($val, JSON_UNESCAPED_UNICODE);
    case 'text':
    default:
      return is_scalar($val) ? sanitize_text_field($val) : wp_json_encode($val, JSON_UNESCAPED_UNICODE);
  }
}
...
function svc_laboratoire_get(WP_REST_Request $req)
{
  ...
  if ($avatar === '' && $uid) {
    ...
    if (!$avatar)
      $avatar = get_user_meta($uid, 'profile_photo', true);
    if (!$avatar)
...
function svc_laboratoire_createOLD(WP_REST_Request $req)
{
  ...
  $ins['directeur_user_id'] = get_current_user_id();
  $formats[] = '%d';
  // Audit
...
function svc_laboratoire_update(WP_REST_Request $req)
{
  ...
  foreach ($allowed as $k => $def) {
    ...
    $val = svc_labo_sanitize($k, $data[$k], $def);
    if ($val === null)
      continue;         // pas de set à NULL par défaut
...
function svc_laboratoire_update_directeur(WP_REST_Request $req)
{
  ...
  $id = absint($req['id'] ?? 0);
  if ($id <= 0)
    return new WP_Error('bad_request', 'ID laboratoire invalide', ['status' => 400]);
  ...
  if (!$u)
    return new WP_Error('not_found', 'Utilisateur (directeur) introuvable', ['status' => 404]);
...
function svc_laboratoire_mine(WP_REST_Request $req)
{
  ...
  elseif (
    in_array('um_service_utm', $roles, true) || in_array('um_service-utm', $roles, true)
...
function svc_membre_create(WP_REST_Request $req)
{
  ...
  foreach ($allowed as $k => $def) {
    ...
    if (!array_key_exists($k, $data)) {
      if ($is_required)
        return new WP_Error('missing_param', "Paramètre requis: $k", array('status' => 400));
      continue;
    }
    $val = svc_membre_sanitize($k, $data[$k], $def);
    if (($val === null || $val === '') && $is_required) {
  ...
  // Defaults + audit
  if (empty($row['api'])) {
    $row['api'] = 'plateforme-recherche/v1';
    $formats[] = '%s';
  }
  if (empty($row['service'])) {
    $row['service'] = 'Espace Labo';
    $formats[] = '%s';
  }
  $row['user_created'] = get_current_user_id() ?: null;
  $formats[] = '%d';
...
function svc_membre_update(WP_REST_Request $req)
{
  ...
  if (empty($upd))
    return new WP_Error('bad_request', 'Aucun champ valide à mettre à jour', array('status' => 400));
...
function svc_membre_list(WP_REST_Request $req)
{
  ...
  if ($uid = $req->get_param('user_id')) {
    $where[] = "m.user_id = %d";
    $params[] = intval($uid);
  }
  if ($g = trim((string) $req->get_param('grade'))) {
    $where[] = "m.grade LIKE %s";
...
";
  ...
  // (Optionnel) ne compter que les comptes actifs si tu as déjà um_status joint plus haut
  // $sql_rep .= " AND (um_status.meta_value IS NULL OR um_status.meta_value = 'active')";
  ...
  $repartition_grade = $repartition_grade ?: array();

  return array(
    'data' => array_map('svc_membre_decode_out', $rows),
    // nouvelle clé attendue par le front
    'repartition_grade' => $repartition_grade,
    // compat (ancienne clé — tu peux la vider ou la mapper si besoin)
    // 'repartition_specialite' => array(),
  );
}
...
// === chercheur ===
function svc_chercheur_table()
{
  global $wpdb;
  return $wpdb->prefix . 'recherche_chercheur';
}
function svc_chercheur_allowed()
{
  return array('email', 'nom', 'prenom', 'grade', 'laboratoire_id', 'orcid', 'photo_url', 'site_web', 'specialite');
}

function svc_chercheur_list(WP_REST_Request $req)
{
  global $wpdb;
  $table = svc_chercheur_table();
  $page = max(1, intval($req->get_param('page') ?: 1));
  $per = max(1, min(200, intval($req->get_param('per_page') ?: 20)));
  $off = ($page - 1) * $per;
...
// === document ===
function svc_document_table()
{
  global $wpdb;
  return $wpdb->prefix . 'recherche_document';
}
function svc_document_allowed()
{
  return array('fichier_path', 'titre', 'chercheur_id', 'date_upload', 'type', 'visibility');
}
...
// === enseignement ===
function svc_enseignement_table()
{
  global $wpdb;
  return $wpdb->prefix . 'recherche_enseignement';
}
function svc_enseignement_allowed()
{
  return array('annee_universitaire', 'ue', 'volume_horaire', 'chercheur_id', 'niveau', 'semestre', 'type');
}

function svc_enseignement_list(WP_REST_Request $req)
{
  global $wpdb;
  $table = svc_enseignement_table();
  $page = max(1, intval($req->get_param('page') ?: 1));
  $per = max(1, min(200, intval($req->get_param('per_page') ?: 20)));
  $off = ($page - 1) * $per;
  $sql = $wpdb->prepare("SELECT * FROM $table ORDER BY id DESC LIMIT %d OFFSET %d", $per, $off);
...
function svc_enseignement_create(WP_REST_Request $req)
{
  global $wpdb;
  $table = svc_enseignement_table();
  $allowed = svc_enseignement_allowed();
  $data = svc_read_input($req);
  $ins = array();
  foreach ($allowed as $k) {
    if (isset($data[$k])) {
...
{
  $u = wp_get_current_user();
  return in_array('um_chercheur', (array) $u->roles, true);
}
...
{
  global $wpdb;
  return $wpdb->prefix . 'recherche_manifestation';
}
...
// === notification ===
function svc_notification_table()
{
  global $wpdb;
  return $wpdb->prefix . 'recherche_notification';
}
function svc_notification_allowed()
{
  return array('lu', 'chercheur_id');
}
/*
...
function svc_notification_create(WP_REST_Request $req)
{
  global $wpdb;
  $table = svc_notification_table();
  $allowed = svc_notification_allowed();
  $data = svc_read_input($req);
  $ins = array();
  foreach ($allowed as $k) {
    if (isset($data[$k])) {
...
// === projet ===
function svc_projet_table()
{
  global $wpdb;
  return $wpdb->prefix . 'recherche_projet';
}
function svc_projet_allowed()
{
  return array('date_debut', 'titre', 'type_projet_id', 'budget', 'chercheur_id', 'date_fin', 'resume', 'statut', 'type_financement', 'objectifs');
}
...
function svc_projet_create(WP_REST_Request $req)
{
  ...
  if (!empty($_FILES['budget_piece'])) {
    $path = svc_projet_handle_file($_FILES['budget_piece'], 'budget');
    if ($path)
      $ins['budget_piece'] = $path;
  }
  if (!empty($_FILES['convention_piece'])) {
    $path = svc_projet_handle_file($_FILES['convention_piece'], 'convention');
    if ($path)
      $ins['convention_piece'] = $path;
  }
...
function svc_projet_update(WP_REST_Request $req)
{
  ...
  if (!empty($_FILES['budget_piece'])) {
    $path = svc_projet_handle_file($_FILES['budget_piece'], 'budget');
    if ($path)
      $upd['budget_piece'] = $path;
  }
  if (!empty($_FILES['convention_piece'])) {
    $path = svc_projet_handle_file($_FILES['convention_piece'], 'convention');
    if ($path)
      $upd['convention_piece'] = $path;
  }
...
function svc_projet_stats(WP_REST_Request $req)
{
  ...
  elseif (in_array('um_chercheur', $roles)) {
    ...
    if ($lab_id) {
      ...
      // fallback = seulement ses projets
      $total = $wpdb->get_var($wpdb->prepare(
        "SELECT COUNT(*) FROM $table WHERE chercheur_id = %d",
        $user_id
      ));
      $financement = $wpdb->get_var($wpdb->prepare(
        "SELECT SUM(budget) FROM $table WHERE chercheur_id = %d",
        $user_id
      ));
      $repartition = $wpdb->get_results($wpdb->prepare(
...
// ======= projet_membre ===
function svc_projet_membre_table()
{
  global $wpdb;
  return $wpdb->prefix . 'recherche_projet_membre';
}
function svc_projet_membre_allowed()
{
  return array('chercheur_id', 'projet_id', 'role_projet');
}

function svc_projet_membre_list(WP_REST_Request $req)
{
  global $wpdb;
  $table = svc_projet_membre_table();
  $page = max(1, intval($req->get_param('page') ?: 1));
  $per = max(1, min(200, intval($req->get_param('per_page') ?: 20)));
  $off = ($page - 1) * $per;
  $sql = $wpdb->prepare("SELECT * FROM $table ORDER BY id DESC LIMIT %d OFFSET %d", $per, $off);
...
function svc_projet_membre_create(WP_REST_Request $req)
{
  global $wpdb;
  $table = svc_projet_membre_table();
  $allowed = svc_projet_membre_allowed();
  $data = svc_read_input($req);
  $ins = array();
  foreach ($allowed as $k) {
    if (isset($data[$k])) {
...
function svc_users_list(WP_REST_Request $req)
{
  ...
  foreach ($rows as &$r) {
    $r['id'] = (int) $r['id'];
    if (isset($r['etablissement_id']))
      $r['etablissement_id'] = (int) $r['etablissement_id'];
  }
...
if (!function_exists('svc_column_exists')) {
  function svc_column_exists($table, $col)
  {
    global $wpdb;
...
if (!function_exists('svc_table_columns')) {
  function svc_table_columns($table)
  {
    global $wpdb;
    static $cache = array();
...
if (!function_exists('svc_find_chercheur_table')) {
  function svc_find_chercheur_table()
  {
    ...
    foreach (array($wpdb->prefix . 'rechercheold_chercheur', $wpdb->prefix . 'recherche_chercheur') as $t) {
      ...
      if ($ok && $wpdb->get_var("SHOW COLUMNS FROM {$t} LIKE 'id'"))
        return $t;
    }
    return null;
  }
...
// === reunion ===
function svc_reunion_table()
{
  global $wpdb;
  return $wpdb->prefix . 'recherche_reunion';
}
if (!defined('ABSPATH'))
  exit;
...
function svc_reseaux_sanitize($fmt, $val)
{
  ...
  switch ($fmt) {
    ...
    case 'url':
      return is_string($val) ? esc_url_raw($val) : null;
    default:
      return is_scalar($val) ? sanitize_text_field($val) : wp_json_encode($val, JSON_UNESCAPED_UNICODE);
  }
}
...
{
  if (class_exists('UTM_Publication_Service'))
    return UTM_Publication_Service::is_director();
  $u = wp_get_current_user();
  return in_array('um_directeur_laboratoire', (array) $u->roles, true);
}
...
function svc_reseaux_fetch_owned($id)
{
  ...
  if (svc_column_exists($t, 'created_by') && (int) $row['created_by'] === get_current_user_id())
    return [$row, null];
  if (svc_is_director())
    return [$row, null];
...
function svc_reseaux_get(WP_REST_Request $req)
{
  ...
  $row = $wpdb->get_row($wpdb->prepare("SELECT * FROM $table WHERE id=%d", $id), ARRAY_A);
  if (!$row)
    return new WP_Error('not_found', 'Réseau introuvable', ['status' => 404]);
...
function svc_reseaux_list(WP_REST_Request $req)
{
  ...
  if (svc_column_exists($t, 'laboratoire_id') && !empty($myLabs)) {
    ...
    if (svc_column_exists($t, 'created_by')) {
      $where[] = "created_by = %d";
      $params[] = get_current_user_id();
    }
  }
...
function svc_reseaux_create(WP_REST_Request $req)
{
  ...
  if (!$labo_id)
    return new WP_Error('no_labo', 'Laboratoire introuvable', array('status' => 403));

  // Upload si présent
  $upload_info = reseaux_store_file_from_upload('piece_jointe');
  if (is_wp_error($upload_info))
    return $upload_info; // erreur upload
  ...
  $ins = array(
    ...
    'logo_url' => $req['logo_url'] ? esc_url_raw($req['logo_url']) : null,
    'avatar_url' => $req['avatar_url'] ? esc_url_raw($req['avatar_url']) : null,
    'contact_tel' => $req['contact_tel'] ? sanitize_text_field($req['contact_tel']) : null,
    'contact_fonction' => $req['contact_fonction'] ? sanitize_text_field($req['contact_fonction']) : null,
  );
...
function svc_reseaux_update(WP_REST_Request $req)
{
  ...
  $id = absint($req['id']);
  if (!$id)
    return new WP_Error('bad_id', 'ID manquant', ['status' => 400]);
  ...
  foreach (['institution', 'pays', 'type_collab', 'contact_nom', 'contact_email', 'date_debut', 'date_fin', 'statut'] as $k) {
    if (isset($req[$k]))
      $data[$k] = ($k === 'contact_email') ? sanitize_email($req[$k]) : sanitize_text_field($req[$k]);
  }

  if (isset($req['site_web']))
    $data['site_web'] = esc_url_raw($req['site_web']);
  if (isset($req['adresse_org']))
    $data['adresse_org'] = sanitize_text_field($req['adresse_org']);

  // Handle image URLs
  if (isset($req['logo_url']))
    $data['logo_url'] = esc_url_raw($req['logo_url']);
  if (isset($req['avatar_url']))
    $data['avatar_url'] = esc_url_raw($req['avatar_url']);

  // Handle additional contact fields
  if (isset($req['contact_tel']))
    $data['contact_tel'] = sanitize_text_field($req['contact_tel']);
  if (isset($req['contact_fonction']))
    $data['contact_fonction'] = sanitize_text_field($req['contact_fonction']);

  if (isset($req['convention_signee']))
    $data['convention_signee'] = absint($req['convention_signee']);
  if (isset($req['projets_associes']))
    $data['projets_associes'] = wp_json_encode(array_map('absint', (array) $req['projets_associes']));
  ...
  if (!$data)
    return new WP_Error('no_fields', 'Aucun champ', ['status' => 400]);
...
// === these ===
function svc_these_table()
{
  global $wpdb;
  return $wpdb->prefix . 'recherche_these';
}
function svc_these_allowed()
{
  return array('date_debut', 'doctorant_nom', 'sujet', 'date_soutenance', 'encadrant_id', 'statut');
}

function svc_these_list(WP_REST_Request $req)
{
  global $wpdb;
  $table = svc_these_table();
  $page = max(1, intval($req->get_param('page') ?: 1));
  $per = max(1, min(200, intval($req->get_param('per_page') ?: 20)));
  $off = ($page - 1) * $per;
  $sql = $wpdb->prepare("SELECT * FROM $table ORDER BY id DESC LIMIT %d OFFSET %d", $per, $off);
...
function svc_reseaux_table()
{
  global $wpdb;
  return $wpdb->prefix . 'recherche_reseaux';
}
function svc_reseaux_projets_table()
{
  global $wpdb;
  return $wpdb->prefix . 'recherche_reseaux_projets';
}
...
function svc_reseaux_args_update()
{
  return svc_reseaux_args_create();
}
...
function svc_reseaux_list_visible(WP_REST_Request $req)
{
  ...
  if ($pays = trim((string) $req['pays'])) {
    $where[] = "AND r.pays = %s";
    $params[] = $pays;
  }
  if ($statut = trim((string) $req['statut'])) {
    $where[] = "AND r.statut = %s";
    $params[] = $statut;
  }
  if ($hc = $req['has_convention']) {
    $where[] = "AND r.convention_signee = %d";
    $params[] = absint($hc);
  }
  if ($df = $req['date_from']) {
    $where[] = "AND r.date_debut >= %s";
    $params[] = $df;
  }
  if ($dt = $req['date_to']) {
    $where[] = "AND COALESCE(r.date_fin,'2999-12-31') <= %s";
    $params[] = $dt;
  }
...
function svc_duration_human($d1, $d2)
{
  ...
  try {
    ...
    if ($diff->y > 0)
      return sprintf('%dan %dmois', $diff->y, $diff->m);
    if ($diff->m > 0)
      return sprintf('%dmois %dj', $diff->m, $diff->d);
...
function svc_depense_create(WP_REST_Request $req)
{
  ...
  // --- Gestion de l'upload de fichier (pièce jointe) ---
  $piece_jointe_url = '';
  if (isset($_FILES['piece_jointe']) && $_FILES['piece_jointe']['error'] === UPLOAD_ERR_OK) {
    // Utilisez media_handle_upload() de WordPress pour uploader (ajustez le chemin si besoin)
    require_once(ABSPATH . 'wp-admin/includes/media.php');
    require_once(ABSPATH . 'wp-admin/includes/file.php');
    require_once(ABSPATH . 'wp-admin/includes/image.php');

    $attachment_id = media_handle_upload('piece_jointe', 0); // 0 = sans post parent
    if (is_wp_error($attachment_id)) {
      return new WP_Error('upload_error', 'Erreur lors de l\'upload du fichier : ' . $attachment_id->get_error_message(), ['status' => 400]);
    }
...
function svc_suivi_projets(WP_REST_Request $req)
{
  ...
  // Cas 1 : Admin ou Service UTM → tous les projets
  if (in_array('administrator', $roles) || in_array('um_service_utm', $roles) || in_array('um_pmo', $roles)) {
    // pas de filtre
  }
  // Cas 2 : Directeur de labo
  elseif (in_array('um_directeur_laboratoire', $roles)) {
...
{
  global $wpdb;
  return $wpdb->prefix . 'recherche_manifestation_categorie';
}
function svc_manifestation_images_table()
{
  global $wpdb;
  return $wpdb->prefix . 'recherche_manifestation_images';
}
...
function svc_manifestation_create(WP_REST_Request $req)
{
  global $wpdb;
  $table = svc_manifestation_table();
  $allowed = svc_manifestation_allowed();
  $data = svc_read_input($req);
  $ins = [];
  $user = wp_get_current_user();
  $roles = $user->roles;
  ...
  // défauts utiles
  if (empty($ins['user_id'])) {
    $ins['user_id'] = get_current_user_id();
  }
  if (empty($ins['statut']))
    $ins['statut'] = 'publie';
  if (empty($ins['date']))
    $ins['date'] = current_time('Y-m-d');
...
{
  global $wpdb;
  $t = svc_manifestation_categorie_table();
...
function svc_manifestation_images_add(WP_REST_Request $req)
{
  ...
  if (!$mid)
    return new WP_Error('bad_id', 'manifestation_id manquant', ['status' => 400]);
  if (empty($_FILES['files']))
    return [];
...
function svc_manifestation_images_list(WP_REST_Request $req)
{
  ...
  $mid = absint($req['id']);
  return $wpdb->get_results($wpdb->prepare(
    "SELECT id, image_url, alt_text, ordre FROM $t WHERE manifestation_id=%d ORDER BY ordre ASC, id ASC",
    $mid
  ), ARRAY_A) ?: [];
}
...
function svc_manifestation_images_delete(WP_REST_Request $req)
{
  ...
  if (!$img)
    return new WP_Error('bad_id', 'image_id manquant', ['status' => 400]);
...
function svc_manifestation_stats(WP_REST_Request $req)
{
  ...
  $nbMonth = (int) $wpdb->get_var($wpdb->prepare(
    "SELECT COUNT(*) FROM $t WHERE statut='publie' AND date BETWEEN %s AND %s",
    $firstDay,
    $lastDay
  ));
...
{
  global $wpdb;
  return $wpdb->prefix . 'recherche_publication';
}
/*
function svc_membre_table(){
...
function svc_publications_list(array $opt)
{
  ...
  $sql_base = "
    ...
    WHERE $whereSQL and statut = 'Validée'
  ";

  $sql_items = "
    SELECT 
      p.id, p.date_publication, p.titre, p.type, p.chercheur_id, p.laboratoire_id,
      p.doi, p.fichier_url, p.isbn, p.revue, p.statut, p.created_by, p.resume, p.commentaire,
  ...
  $items = $wpdb->get_results(
    $wpdb->prepare($sql_items, ...array_merge($bind, [$per, $off])),
    ARRAY_A
  );
...
function svc_reseaux_list2(array $opt)
{
  ...
  if ($opt['type_collab'] !== '') {
    $where[] = "r.type_collab = %s";
    $bind[] = $opt['type_collab'];
  }
  if ($opt['search'] !== '') {
    $like = '%' . $wpdb->esc_like($opt['search']) . '%';
  ...
  foreach ($items as &$r) {
    ...
    if (!is_array($r['projets_associes']))
      $r['projets_associes'] = [];
  }
...
function svc_manifestation_list_by_lab_route(WP_REST_Request $req)
{
  ...
  if (!$opt['laboratoire_id'])
    return new WP_Error('bad_request', 'laboratoire_id requis', ['status' => 400]);
...
function svc_manifestation_list_by_lab(array $opt)
{
  ...
  if (!empty($opt['categorie_id'])) {
    $where[] = "m.categorie_id = %d";
    $bind[] = (int) $opt['categorie_id'];
  }
  if (!empty($opt['type'])) {
    $where[] = "m.type = %s";
    $bind[] = $opt['type'];
  ...
  if (!empty($opt['date_from'])) {
    $where[] = "COALESCE(m.date_debut, m.`date`) >= %s";
    $bind[] = $opt['date_from'];
  }
  if (!empty($opt['date_to'])) {
    $where[] = "COALESCE(m.date_fin, m.date_debut, m.`date`) <= %s";
    $bind[] = $opt['date_to'];
  }
