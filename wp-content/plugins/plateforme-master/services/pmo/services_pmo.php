...
{
  global $wpdb;
  return $wpdb->prefix . 'pmo_document';
}
function svc_document_allowed()
{
  return array('categorie', 'piece_jointe', 'titre', 'visibilite');
}
...
// === document_download ===
function svc_document_download_table()
{
  global $wpdb;
  return $wpdb->prefix . 'pmo_document_download';
}
function svc_document_download_allowed()
{
  return array();
}
...
{
  global $wpdb;
  $table = svc_document_download_table();
  $page = max(1, intval($req->get_param('page') ?: 1));
  $per = max(1, min(200, intval($req->get_param('per_page') ?: 20)));
  $off = ($page - 1) * $per;
  $sql = $wpdb->prepare("SELECT * FROM $table ORDER BY id DESC LIMIT %d OFFSET %d", $per, $off);
...
function svc_document_download_create(WP_REST_Request $req)
{
  global $wpdb;
  $table = svc_document_download_table();
  $allowed = svc_document_download_allowed();
  $data = svc_read_input($req);
  $ins = array();
  foreach ($allowed as $k) {
    if (isset($data[$k])) {
...
// === document_export ===
function svc_document_export_table()
{
  global $wpdb;
  return $wpdb->prefix . 'pmo_document_export';
}
function svc_document_export_allowed()
{
  return array();
}
...
{
  global $wpdb;
  $table = svc_document_export_table();
  $page = max(1, intval($req->get_param('page') ?: 1));
  $per = max(1, min(200, intval($req->get_param('per_page') ?: 20)));
  $off = ($page - 1) * $per;
  $sql = $wpdb->prepare("SELECT * FROM $table ORDER BY id DESC LIMIT %d OFFSET %d", $per, $off);
...
function svc_document_export_create(WP_REST_Request $req)
{
  global $wpdb;
  $table = svc_document_export_table();
  $allowed = svc_document_export_allowed();
  $data = svc_read_input($req);
  $ins = array();
  foreach ($allowed as $k) {
    if (isset($data[$k])) {
...
// === organisation_membre ===
function svc_organisation_membre_table()
{
  global $wpdb;
  return $wpdb->prefix . 'pmo_organisation_membre';
}
function svc_organisation_membre_allowed()
{
  return array('email', 'nom_complet', 'role');
}

function svc_organisation_membre_list(WP_REST_Request $req)
{
  global $wpdb;
  $table = svc_organisation_membre_table();
  $page = max(1, intval($req->get_param('page') ?: 1));
  $per = max(1, min(200, intval($req->get_param('per_page') ?: 20)));
  $off = ($page - 1) * $per;
  $sql = $wpdb->prepare("SELECT * FROM $table ORDER BY id DESC LIMIT %d OFFSET %d", $per, $off);
...
function svc_organisation_membre_create(WP_REST_Request $req)
{
  global $wpdb;
  $table = svc_organisation_membre_table();
  $allowed = svc_organisation_membre_allowed();
  $data = svc_read_input($req);
  $ins = array();
  foreach ($allowed as $k) {
    if (isset($data[$k])) {
...
// === plateforme ===
function svc_plateforme_table()
{
  global $wpdb;
  return $wpdb->prefix . 'pmo_plateforme';
}
function svc_plateforme_allowed()
{
  return array('domaine', 'nom', 'responsable_id');
}

function svc_plateforme_list(WP_REST_Request $req)
{
  global $wpdb;
  $table = svc_plateforme_table();
  $page = max(1, intval($req->get_param('page') ?: 1));
  $per = max(1, min(200, intval($req->get_param('per_page') ?: 20)));
  $off = ($page - 1) * $per;
  $sql = $wpdb->prepare("SELECT * FROM $table ORDER BY id DESC LIMIT %d OFFSET %d", $per, $off);
...
function svc_plateforme_create(WP_REST_Request $req)
{
  global $wpdb;
  $table = svc_plateforme_table();
  $allowed = svc_plateforme_allowed();
  $data = svc_read_input($req);
  $ins = array();
  foreach ($allowed as $k) {
    if (isset($data[$k])) {
...
// === plateforme_document ===
function svc_plateforme_document_table()
{
  global $wpdb;
  return $wpdb->prefix . 'pmo_plateforme_document';
}
function svc_plateforme_document_allowed()
{
...
{
  global $wpdb;
  $table = svc_plateforme_document_table();
  $page = max(1, intval($req->get_param('page') ?: 1));
  $per = max(1, min(200, intval($req->get_param('per_page') ?: 20)));
  $off = ($page - 1) * $per;
  $sql = $wpdb->prepare("SELECT * FROM $table ORDER BY id DESC LIMIT %d OFFSET %d", $per, $off);
...
function svc_plateforme_document_create(WP_REST_Request $req)
{
  global $wpdb;
  $table = svc_plateforme_document_table();
  $allowed = svc_plateforme_document_allowed();
  $data = svc_read_input($req);
  $ins = array();
  foreach ($allowed as $k) {
    if (isset($data[$k])) {
...
// === plateforme_export ===
function svc_plateforme_export_table()
{
  global $wpdb;
  return $wpdb->prefix . 'pmo_plateforme_export';
}
function svc_plateforme_export_allowed()
{
  return array();
}
...
{
  global $wpdb;
  $table = svc_plateforme_export_table();
  $page = max(1, intval($req->get_param('page') ?: 1));
  $per = max(1, min(200, intval($req->get_param('per_page') ?: 20)));
  $off = ($page - 1) * $per;
  $sql = $wpdb->prepare("SELECT * FROM $table ORDER BY id DESC LIMIT %d OFFSET %d", $per, $off);
...
function svc_plateforme_export_create(WP_REST_Request $req)
{
  global $wpdb;
  $table = svc_plateforme_export_table();
  $allowed = svc_plateforme_export_allowed();
  $data = svc_read_input($req);
  $ins = array();
  foreach ($allowed as $k) {
    if (isset($data[$k])) {
...
// === pmo_dashboard ===
function svc_pmo_dashboard_table()
{
  global $wpdb;
  return $wpdb->prefix . 'pmo_pmo_dashboard';
}
function svc_pmo_dashboard_allowed()
{
  return array();
}
...
function svc_pmo_dashboard_create(WP_REST_Request $req)
{
  global $wpdb;
  $table = svc_pmo_dashboard_table();
  $allowed = svc_pmo_dashboard_allowed();
  $data = svc_read_input($req);
  $ins = array();
  foreach ($allowed as $k) {
    if (isset($data[$k])) {
...
// === pmo_report ===
function svc_pmo_report_table()
{
  global $wpdb;
  return $wpdb->prefix . 'pmo_pmo_report';
}
function svc_pmo_report_allowed()
{
  return array();
}
...
function svc_pmo_report_create(WP_REST_Request $req)
{
  global $wpdb;
  $table = svc_pmo_report_table();
  $allowed = svc_pmo_report_allowed();
  $data = svc_read_input($req);
  $ins = array();
  foreach ($allowed as $k) {
    if (isset($data[$k])) {
...
// === presentation_ceip ===
function svc_presentation_ceip_table()
{
  global $wpdb;
  return $wpdb->prefix . 'pmo_presentation_ceip';
}
function svc_presentation_ceip_allowed()
{
  return array();
}
...
{
  global $wpdb;
  $table = svc_presentation_ceip_table();
  $page = max(1, intval($req->get_param('page') ?: 1));
  $per = max(1, min(200, intval($req->get_param('per_page') ?: 20)));
  $off = ($page - 1) * $per;
  $sql = $wpdb->prepare("SELECT * FROM $table ORDER BY id DESC LIMIT %d OFFSET %d", $per, $off);
...
function svc_presentation_ceip_create(WP_REST_Request $req)
{
  global $wpdb;
  $table = svc_presentation_ceip_table();
  $allowed = svc_presentation_ceip_allowed();
  $data = svc_read_input($req);
  $ins = array();
  foreach ($allowed as $k) {
    if (isset($data[$k])) {
...
function svc_presentation_ceip_publish_table()
{
  global $wpdb;
  return $wpdb->prefix . 'pmo_presentation_ceip_publish';
}
function svc_presentation_ceip_publish_allowed()
{
  return array();
}
...
{
  global $wpdb;
  $table = svc_presentation_ceip_publish_table();
  $page = max(1, intval($req->get_param('page') ?: 1));
  $per = max(1, min(200, intval($req->get_param('per_page') ?: 20)));
  $off = ($page - 1) * $per;
  $sql = $wpdb->prepare("SELECT * FROM $table ORDER BY id DESC LIMIT %d OFFSET %d", $per, $off);
...
function svc_presentation_ceip_publish_create(WP_REST_Request $req)
{
  global $wpdb;
  $table = svc_presentation_ceip_publish_table();
  $allowed = svc_presentation_ceip_publish_allowed();
  $data = svc_read_input($req);
  $ins = array();
  foreach ($allowed as $k) {
    if (isset($data[$k])) {
...
// === requete ===
function svc_requete_table()
{
  global $wpdb;
  return $wpdb->prefix . 'pmo_requete';
}
function svc_requete_allowed()
{
  return array('priorite', 'statut', 'titre', 'type');
}

function svc_requete_list(WP_REST_Request $req)
{
  global $wpdb;
  $table = svc_requete_table();
  $page = max(1, intval($req->get_param('page') ?: 1));
  $per = max(1, min(200, intval($req->get_param('per_page') ?: 20)));
  $off = ($page - 1) * $per;
  $sql = $wpdb->prepare("SELECT * FROM $table ORDER BY id DESC LIMIT %d OFFSET %d", $per, $off);
...
// === requete_export ===
function svc_requete_export_table()
{
  global $wpdb;
  return $wpdb->prefix . 'pmo_requete_export';
}
function svc_requete_export_allowed()
{
  return array();
}
...
{
  global $wpdb;
  $table = svc_requete_export_table();
  $page = max(1, intval($req->get_param('page') ?: 1));
  $per = max(1, min(200, intval($req->get_param('per_page') ?: 20)));
  $off = ($page - 1) * $per;
  $sql = $wpdb->prepare("SELECT * FROM $table ORDER BY id DESC LIMIT %d OFFSET %d", $per, $off);
...
function svc_requete_export_create(WP_REST_Request $req)
{
  global $wpdb;
  $table = svc_requete_export_table();
  $allowed = svc_requete_export_allowed();
  $data = svc_read_input($req);
  $ins = array();
  foreach ($allowed as $k) {
    if (isset($data[$k])) {
...
function svc_requete_piece_jointe_table()
{
  global $wpdb;
  return $wpdb->prefix . 'pmo_requete_piece_jointe';
}
function svc_requete_piece_jointe_allowed()
{
  return array('fichier');
}
...
{
  global $wpdb;
  $table = svc_requete_piece_jointe_table();
  $page = max(1, intval($req->get_param('page') ?: 1));
  $per = max(1, min(200, intval($req->get_param('per_page') ?: 20)));
  $off = ($page - 1) * $per;
  $sql = $wpdb->prepare("SELECT * FROM $table ORDER BY id DESC LIMIT %d OFFSET %d", $per, $off);
...
function svc_requete_piece_jointe_create(WP_REST_Request $req)
{
  global $wpdb;
  $table = svc_requete_piece_jointe_table();
  $allowed = svc_requete_piece_jointe_allowed();
  $data = svc_read_input($req);
  $ins = array();
  foreach ($allowed as $k) {
    if (isset($data[$k])) {
