...
function profile_get($current_user_id) {
  ...
  $profil = [
    ...
    'email1'      => $user->user_email, // wp_users
    'email2'      => _meta($current_user_id,'email2',''),
    'cin'         => _meta($current_user_id,'cin',''),
    'cv'          => _meta($current_user_id,'cv_url',''),
    'avatar'      => _meta($current_user_id,'avatar_url','') ?: get_avatar_url($current_user_id),
  ];

  // Récup IDs grade/spécialité (toujours renvoyés si présents)
  $grade_id = (int)_meta($current_user_id,'grade_id',0);
  $spec_id  = (int)_meta($current_user_id,'specialite_id',0);
  if ($grade_id) { $profil['grade'] = profile__grade_row($grade_id); }
  if ($spec_id)  { $profil['specialite'] = profile__spec_row($spec_id); }
  ...
  if ($is_student) {
    ...
    $profil['academic_info'] = _meta($current_user_id,'academic_info',''); // texte/JSON éventuel
  }
...
function profile_upload_cv($current_user_id, $payload) {
  ...
  list($dir,$baseurl) = profile__uploads_dir();
  $ext  = (strpos($mime,'pdf')!==false) ? 'pdf' : 'bin';
