...
function addParticipantToEditList(input, listElement) {
 ...
 if (!emailRegex.test(email)) {
  confirm('Adresse email invalide');
  return;
 }
 ...
 if (exists) {
  confirm('Cet email est déjà ajouté');
  return;
 }
 ...
 saveBtn.addEventListener('click', async () => {
  ...
  try {
   ...
   if (!res.ok) {
    ...
    confirm(data.message || 'Erreur inconnue');
    return;
   }

   confirm('Réunion créée avec succès');
   addModal.style.display = 'none';
   loadReunions();
  } catch (e) {
   confirm('Impossible de créer la réunion');
 ...
 // const addEditParticipantBtn = document.getElementById('addEditParticipantBtn');
  ...
  updateBtn.addEventListener('click', async () => {
   ...
   try {
    ...
    const data = await res.json();
    if (!res.ok) {
     confirm(data.message || 'Échec de la mise à jour');
     return;
    }

    confirm('Réunion mise à jour avec succès');
    closeEditModal();
    loadReunions();
   } catch (e) {
    confirm('Impossible de mettre à jour la réunion');
    console.error(e);
   }
  });
